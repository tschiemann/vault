...
type ConflictResolver interface {
	...
	ResolveAliases(ctx context.Context, existing, duplicate *identity.Alias) error
}
...
// returns a duplicate name error, which should be handled by the caller by
// putting the system in case-sensitive mode.
func (r *errorResolver) ResolveAliases(ctx context.Context, existing, duplicate *identity.Alias) error {
	if existing == nil {
		return nil
	}
